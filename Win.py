        ...
        while True:
            ...
            if key is None:continue
                ...
                if out == "exit":return
                    ...
                    maxlength = 0
                    for x in save_dir.keys():
                        maxlength = max(len(x) + 2,maxlength)
                    for x,y in save_dir.items():
                        print(f"{x}{' ' * (maxlength - len(x))}: {y}")
                elif out[:4] == "cds ":
                    save_dir[out[4:]] = cwd
                elif out[:3] == "cd ":
                    try:
                ...
                if history_counter > 0:
                    print("\b" * edit_counter, end=" " * len(data) + "\b" * len(data))
                    history_counter -= 1
